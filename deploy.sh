...
cd "$SCRIPT_DIR"
xbuild ImpatientCommuters.csproj /p:Configuration="$CONFIGURATION" /nologo /verbosity:quiet
...
rm -rf "$DIST"
mkdir -p "$DIST"
cp "$BUILD_OUT/ImpatientCommuters.dll" "$DIST/ImpatientCommuters.dll"
echo ""
echo "Staged to: $DIST"
ls -lh "$DIST"
...
cp "$BUILD_OUT/CitiesHarmony.API.dll"       "$MODS_DIR/"
cp "$BUILD_OUT/CitiesHarmony.Harmony.dll"   "$MODS_DIR/"
